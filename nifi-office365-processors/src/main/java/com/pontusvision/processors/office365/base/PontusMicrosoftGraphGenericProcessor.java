...
import static com.pontusvision.processors.office365.base.PontusMicrosoftGraphBaseProcessor.writeFlowFile;

@Tags({ "GRAPH", "Generic", "Microsoft", "Office 365" }) @CapabilityDescription("Get messages")

public class PontusMicrosoftGraphGenericProcessor extends AbstractProcessor
{
  ...
  final static PropertyDescriptor DATA = new PropertyDescriptor
    ...
    .addValidator(new StandardValidators.StringLengthValidator(0,1000000000))
    .required(false)
    .expressionLanguageSupported(ExpressionLanguageScope.FLOWFILE_ATTRIBUTES)
    .build();
  ...
  public static final Relationship SUCCESS = PontusMicrosoftGraphBaseProcessor.SUCCESS;


  public static final Relationship FAILURE = PontusMicrosoftGraphBaseProcessor.FAILURE;
...
//    session.remove(flowFile);
    ...
    if (StringUtils.isNotEmpty(functionOptionsStr)) {
    ...
    if (obj != null){
      writeFlowFile(flowFile,session,obj.toString(),SUCCESS );
    }
  ...
  //      }
    ...
    catch (Exception e)
    {
      ...
      {
        PontusMicrosoftGraphBaseProcessor.handleError(getLogger(), ex2, session, flowFile );
      }
    }
